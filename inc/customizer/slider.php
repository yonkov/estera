...
function estera_slider_customize_register( $wp_customize ) {
    ...
    array(
        ...
        'type'        => 'select',
        'choices'               => array(
            'from-post' => __( 'Posts', 'estera' ),
            'from-product' => __( 'Products', 'estera' ),
        ),
        'description'   => esc_html__( 'Choose between post slider or product slider. If you want to display products, you need to have WooCommerce plugin installed and configured.', 'estera' )
        )
    );
	...
	$wp_customize->add_control( 'home_slider_category', array(
        ...
        /* Show only when post slider is selected */
	...
	$wp_customize->add_control( 'home_slider_woo_category', array(
        ...
        /* Show only when product slider is selected */
	...
	$wp_customize->add_setting( 'home_slider_excerpt_size',
		array(
            'default'           => 25,
            'sanitize_callback' => 'absint'
		)
...
function estera_customize_swiper_slider (){
    ...
    .swiper-button-prev, .swiper-button-next {
		display: <?php echo esc_attr($slider_arrows); ?>;        
    }
	.image-overlay {
    ...
    .header-slider-item {
        background-size: <?php echo esc_attr($slider_size); ?> !important;
		background-position: <?php echo esc_attr($slider_position); ?> !important;
    }
    <?php if ($slider_animation) : ?>
    .header-slider-item {
    ...
    <script>
        var mySwiper = new Swiper('.header-slider-wrapper', {
            ...
            simulateTouch: <?php echo esc_attr($auto_drag); ?>,
        });
...
function estera_slide_woo_cats( ) {
	...
	foreach ( get_categories(array('taxonomy'   =>  'product_cat' )) as $categories => $category ) {
		$cats[ $category->term_id ] = $category->name;
	}
	return $cats;
